...
def read_sys(i):
    ...
    if not os.path.exists(rdngfile) or False:
...
maxmix = [10, 10, 10, 10, 10, 10]

# Prepare array
hist_radangori = np.zeros(
    [len(info_systems), rad_N, ang_N, ori_N], dtype=float)
hist_maxcoords = np.zeros(
    [len(info_systems), np.max(maxmix), 3], dtype=float)
