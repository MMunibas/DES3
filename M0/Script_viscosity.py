...
if not os.path.exists(os.path.join(resdir, 'viscosity_results.npz')) or False:
...
axs.set_title('Setup M0', fontweight='bold')
...
# Figure arrangement
figsize = (9, 6)
sf = figsize[0]/figsize[1]

left = 0.15
bottom = 0.15
row = np.array([0.75, 0.00])
column = np.array([0.75/sf, 0.00])
...
figtitle = os.path.join(resdir, 'viscosity_paper_m0.png')
plt.savefig(figtitle, format='png', dpi=dpi)
plt.close()
