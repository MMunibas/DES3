...
chrmdir = {
    ...
    'single_scn': os.path.join(maindir, "../../dev-release-1rkhs-fmdcm-hotfix"),
...
for sys in systems:
    ...
    for ir, residue in enumerate(residues):
        ...
        if 'tip4'.lower() in residf[ir].lower():
            genlines += "generate {:s} noangl nodihe\n\n".format(
                reslab[ir])
        else:
            genlines += (
