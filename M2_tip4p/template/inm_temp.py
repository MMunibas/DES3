...
for istep, file_i in enumerate(dcdfiles):
    ...
    for sys_i in systems:
        ...
        for itask in range(Ntpdf):
            ...
            inplines = inplines.replace('CHRMDIR', '{:s}'.format(chrmdir))
            ...
            if not os.path.exists(dcdname.format(ifile + 1)):
                ...
                while not ready:
                    ...
                    # Check if directory python file exists (directory deleted)
                    if not os.path.exists("inm.py"):
                        exit()
