...
!--------------------------------------------------------------------------------------------
! THIS ROUTINE IMPLEMENTS THE DISTRIBUTED CHARGE MODEL (MIKE DEVEREUX,
! SHAMPA SANTRA,
! MARKUS MEUWLY, JCTC 2014) WHERE OFF-CENTERED CHARGES ARE USED TO ADD
! MULTIPOLAR
! ELECTROSTATICS TO MD SIMULATIONS
!--------------------------------------------------------------------------------------------
!
! DEVELOPER NOTE: BE CAREFUL USING CHARMM'S ROUTINES TO CALCULATE THE
! VIRIAL. THE VIRIAL
! REQUIRES THAT ATOMIC POSITIONS BE MULTIPLIED BY FORCES AT THAT
! POSITION, THE DCM ROUTINES
! TRANSFER FORCES FROM VIRTUAL SITES TO NUCLEAR SITES SO VIRIAL
! CALCULATIONS MUST BE
! CONSISTENT (I.E. CALCULATE VIRIAL AFTER FORCES HAVE BEEN TRANSFERED TO
! NUCLEI IN CENTRAL
...
!
! TEST SUITE:
! A COMPREHENSIVE TEST SUITE FOR DEVELOPMENT PURPOSES IS AVAILABLE FOR
! DOWNLOAD FROM
...
! GLOBAL VARIABLES:
!
! QDCM                  FLAG TO INDICATE TO EXTERNAL ROUTINES THAT DCM
! WILL BE USED
! DCXYZ                 FLAG TO INDICATE THAT AN XYZ FILE OF DCM CHARGES
! SHOULD BE WRITTEN
! DCMUL                 FLAG TO INDICATE THAT MULTIPOLES OF DCM RESIDUES
! SHOULD BE CALCULATED
! (T)SHIFT ETC          FLAGS STORING WHAT TYPE OF CUTOFF SCHEME WILL BE
! USED (SEE NOTES BELOW)
! QTI                   FLAG TO INDICATE THERMODYNAMIC INTEGRATION WILL
! BE PERFORMED
! QPOL                  FLAG TO STATE AT LEAST ONE POLARIZABLE ATOM IS
! PRESENT IN SYSTEM
! QIMG                  WHETHER THIS CALL IS FOR IMAGE ATOMS OR CENTRAL
! CELL
! QWARNED               TO ENSURE WE DON'T THROW EXCESSIVE WARNINGS TO
! OUTPUT
! QDCNB                 WHETHER WE NEED TO UPDATE THE DCM NONBONDED
! LISTS
! IQDCNB                WHETHER WE NEED TO UPDATE THE DCM IMAGE
! NONBONDED LISTS
! QDCNB14               WHETHER WE NEED TO UPDATE THE DCM 1-4 EXCLUSION
! LIST FOR PME
! QFNB                  WHETHER WE NEED TO UPDATE THE FIELD NONBOND LIST
! FOR POLARIZATION TERM
!
! IUDCM                 FILE UNIT FOR DCM INPUT
! IUDCXYZ               FILE UNIT FOR DCM CHARGE XYZ FILE
! IUMUL                 FILE UNIT TO WRITE MULTIPOLE MOMENTS OF DCM
! RESIDUES
...
! NATOMA                NO. OF ATOMS IN PRIMARY CELL (FOR USE WITH
! PERIODIC BOUNDARIES)
! NATOMX1               NO. OF ATOMS IN PRIMARY CELL
! NATIMX                NO. OF ATOMS INCLUDING IMAGES
! MAXQ                  MAXIMUM ALLOWED DCM CHARGES PER ATOM
! MAXPOL                MAXIMUM ALLOWED POLARIZABLE SITES PER ATOM
! NALL                  NO. OF DCM CHARGES IN CURRENT ENERGY EVALUATION
! (CENTRAL CELL OR IMAGES)
! NALLC                 NO. OF DCM CHARGES IN CENTRAL CELL ONLY
! NQ                    NO. CHARGES FOR EACH ATOM IN EACH FRAME (ONE
! ATOM CAN APPEAR IN MORE
!                       THAN ONE FRAME, CHARGES MAY ONLY APPEAR IN ONE
!                       OF THESE FRAMES)
! NQA                   NO. CHARGES BELONGING TO EACH ATOM IN TOTAL
! IDQ                   ARRAY LINKING ATOM NO. TO CORRESPONDING POSITION
! IN ARRAY OF
!                       DCM CHARGES
! FRAMEI                ARRAY LINKING FRAME NUMBER TO CORRESPONDING ATOM
! NUMBERS IN FRAME
! AFRAME                ARRAY LINKING ATOM TO FRAME WHERE IT HAS NONZERO
! CHARGE
! DPAIR                 ARRAY OF GEOMETRIC PAIRS OF DCM CHARGES
! (A1,B1,C1=-A2,-B2,-C2) (LEGACY...)
! IREF                  INDEX OF REFERENCE ATOM IN CENTRAL CELL FOR EACH
! IMAGE ATOM
! ATQ                   ARRAY LINKING DCM CHARGES TO ATOMS THEY BELONG
! TO
! FRTYP                 ARRAY STORING WHETHER EACH FRAME USES BISECTOR
! Z-AXIS OR BOND Z-AXIS
! INBLC,JNBLC           COPY OF INBL,JNBL NONBONDED LISTS FOR CENTRAL
! CELL ONLY
! DNNB14                NO. OF DCM CHARGE 1-4 EXCLUSION INTERACTIONS FOR
! PME
! DINB14,DIBLO14        CHARMM INB14,DIBLO14 EXCLUSION LIST EQUIVALENT
! FOR DCM CHARGES
! DINBL,DJNBL           CHARMM INBL,JNBL NONBONDED LIST EQUIVALENT FOR
! DCM CHARGES
! IDINBL,IDJNBL         CHARMM INBL,JNBL NONBONDED LIST EQUIVALENT FOR
! DCM CHARGES IN IMAGES
!
! PATCH VARIABLES (TO HANDLE PATCH ALTERATIONS TO DCM RESIDUES)
! MOST ARE SET IN GENPSF.SRC
!
! DCMPATF               ARRAY OF PATCH ATOM SEGID, RESID, ATOM NO. IN
! RESIDUE FOR 'FIRST' PATCHES
! DCMPATL               ARRAY OF PATCH ATOM SEGID, RESID, ATOM NO. IN
! RESIDUE FOR 'LAST' PATCHES
...
! MAXRQ                 MAXIMUM DISTANCE FROM A NUCLEUS TO ITS DCM
! CHARGE
! TOKCAL                CONSTANT TO CONVERT ENERGY TO KCAL/MOL
! DCLAMBDA              ENERGY SCALING FACTOR FOR T.I. WITHOUT PERT
! MODULE
! AP                    POLARIZATION DAMPING CONSTANT
! DCEWVIRIAL            TO HOLD DCM PME VIRIAL CORRECTION
! EPOL                  VARIABLE TO HOLD CURRENT POLARIZATION ENERGY FOR
! PRINTING LATER
! AQ,BQ,CQ              LOCAL AXIS DCM CHARGE COORDINATES
! DQ                    DCM CHARGES
! DPOL                  ATOMIC DIPOLAR POLARIZABILITIES
! CGBAK                 TO SAVE ORIGINAL CHARMM CHARGE ARRAY FOR T.I.
! LAMBDA SWITCHING
! XQAC                  STORE DCM CHARGE COORDINATES OF CENTRAL CELL
! ONLY FOR FIELD EVALUATIONS
! B0AQ ETC              STORE AQ FOR LAMBDA=0 STATE (THERMODYNAMIC
! INTEGRATION)
! B1AQ ETC              STORE AQ FOR LAMBDA=1 STATE (THERMODYNAMIC
! INTEGRATION)
! DF1 ETC               GEOMETRIC DERIVATIVES FOR EACH DCM AXIS FRAME
!
! EWALD VARIABLES
! KDCDXA/YA/ZA          ARRAY TO STORE K-SPACE FORCES TO PROVIDE CORRECT
! VIRIAL
!
! HALGREN VARIABLES
! HALGTPY               CHARMM ATOM TYPE (NUMERIC VALUE) THAT WILL USE
! 14-7 TERM
!
!--------------------------------------------------------------------------------------------
! MPI:
! REAL MPI WORK IS DONE ELSEWHERE, NONBOND LIST ARRIVES HERE ALREADY
...
! THE VARIOUS ARRAYS NEED TO BE ENLARGED TO ACCOMMODATE THE ADDITIONAL
! IMAGE ATOMS, THIS IS
! DONE IN IMGINIT. THE DCME ROUTINE IS CALLED TWICE FOR EACH ENERGY
! EVALUATION, ONCE FOR THE
! PRIMARY CELL AND ONCE FOR THE IMAGE ATOMS. IMAGE ATOM CALLS ARE
! RECOGNIZED BY THE NATOMX VS
! NATIMX COMPARISON OR BY THE QIMG FLAG.
!--------------------------------------------------------------------------------------------
! SUPPORTED OPTIONS:
!
! SMOOTH CUTOFFS: SPECIFICALLY SHIFT, FSHIFT AND SWITCH FUNCTIONS.
! PARAMETERS SUCH AS CUTOFF
! DISTANCE ARE TAKEN FROM CHARMM'S NONBONDED INPUT SPECIFICATION, BUT
! WHICH FUNCTION IS USED
! FOR DCM MUST BE SPECIFIED IN THE DCM SECTION OF THE CHARMM INPUT FILE.
! THE "TSHIFT" VS
! "SHIFT" DISTINCTION RELATES TO WHETHER WE USE THE NUCLEAR-NUCLEAR
! DISTANCE TO EVALUATE THE
! CUTOFF FUNCTION FOR ALL CHARGES BELONGING TO A GIVEN ATOM ("SHIFT"),
! OR WHETHER WE USE THE
! CHARGE-CHARGE DISTANCE DIRECTLY ("TSHIFT", PREFERRED)
!
! PARTICLE MESH EWALD SUMMATION SUPPORT HAS BEEN ADDED BY CALLING
! CHARMM'S INTERNAL REAL
! AND K-SPACE ROUTINES FROM THE DCM ROUTINE AFTER CONSTRUCTING
! APPROPRIATE EXCLUSION LISTS
! AND CHARGE AND COORDINATE ARRAYS. AN INTERNAL VIRIAL CORRECTION TERM
! IS NECESSARY
!
! DIPOLAR POLARIZABILITIES ARE IMPLEMENTED WITH DCM CHARGES, BUT ARE NOT
! YET COMPATIBLE WITH
! K-SPACE TERMS FOR PARTICLE MESH EWALD
!
! THERMODYNAMIC INTEGRATION IS POSSIBLE USING CHARMM'S "PERT" MODULE. AS
! WELL AS THE USUAL
! PERT KEYWORDS, THE INPUT FILE MUST DEFINE LAMBDA=0 AND LAMBDA=1 DCM
! STATES USING A
! SEPARATE PARAMETER FILE FOR EACH STATE
!
! PERIODIC BOUNDARIES: AUTOMATICALLY ACTIVATED WHEN PERIODIC BOUNDARIES
! ARE SET UP IN THE
! CHARMM INPUT FILE
!
!--------------------------------------------------------------------------------------------
...
!*******************************************************************************************
   ...
   SUBROUTINE DCM
      ...
      IF (.NOT. ALLOCATED(SELAT)) THEN
         ...
         CALL CHMALLOC('DCM.SRC', 'DCM', 'SELDIP', NATOM, INTG=SELDIP) !CTO HOLD WHICH ATOMS WE USE TO EVALUATE DIPOLE MOMENT
         SELAT(1:NATOM) = 1  ! SELECT ALL ATOMS FOR XYZ FILE BY DEFAULT
         SELDIP(1:NATOM) = 1 ! SELECT ALL ATOMS FOR DIPOLE FILE BY DEFAULT
      END IF

! WARN/DIE IF DCM MODULE WAS ALREADY INITIALIZED AND NOT DOING
...
!        OPEN(UNIT=IUDCXYZ,FILE='DCM.XYZ',STATUS='REPLACE')
               ...
               ! ACTIVATE HALGREN 14-7 REPULSION FOR A SINGLE ATOM TYPE
               ! (MEANT FOR IAMOEBA H2O)
...
!*******************************************************************************************
!
! ROUTINE TO CALCULATE THE DCM ELECTROSTATIC ENERGY AND FORCES OF THE
! SYSTEM
! ENERGY IS ADDED TO ETERM(ELEC) FOR PRIMARY CELL, ETERM(IMELEC) FOR
! IMAGE ATOMS
...
!  USE NEW_TIMER,ONLY:TIMER_START,TIMER_STOP,T_DCM
      ...
      USE ENERGYM, ONLY: ETERM, QETERM, EWKSUM, EWSELF, EWQCOR, EWUTIL, EWEXCL, EPRESS, VIXX, VIZZ, VEXX, VEZZ
...
#if KEY_PARALLEL==1
      USE PARALLEL, ONLY: COMM_CHARMM, MYNOD
      USE MPI, ONLY: MPI_REAL8, MPI_IN_PLACE, MPI_INTEGER, MPI_SUM
!,MPI_COMM_WORLD
#endif
      IMPLICIT NONE
...
!  RNQ              NUMBER OF CHARGES FOR EACH ATOM IN EACH FRAME IN
!  EACH REFERENCE RESIDUE
!  RFRAMEI          ATOM INDEX OF EACH ATOM IN EACH FRAME OF EACH
!  REFERENCE RESIDUE
...
!  XEI,XEJ,XEK      I,J,K COMPONENTS OF LOCAL X-AXIS VECTOR IN GLOBAL
!  COORDINATES
!  DXEIDA1X         PARTIAL DERIVATIVE OF LOCAL X-AXIS I-COMPONENT WRT
!  CHANGE IN X-COORD
...
!  print*,'starting dcme',MYNOD,NATOMX,NATIMX,NATOMX1,QIMG
!  CALL TIMER_START(T_DCM)

      NATOM0 = NATOMX
      IF (NATIMX .EQ. NATOMA) NATIMX = 0 !CATCH PERIODIC SYSTEM WITH NO IMAGE ATOMS WITHIN CUTOFFS
      IF (QDCM) THEN

         IF (NATOMX .EQ. 0) THEN
            ! PROBABLY UNNECESSARY, NATOMX IS ALWAYS # ATOMS IN SYSTEM,
            ! ONLY # ATOMS IN NONBONDED
...
!     CALL WRNDIE(0,'<DCM> DCME','ERROR: DIPOLAR POLARIZATION WITH EWALD
!     SUMMATION IS NOT YET IMPLEMENTED')
!    ENDIF
         ...
         IF (QPOL) THEN ! DETERMINE SIZE OF NONBONDED ARRAY (MIGHT BE ALREADY STORED SOMEWHERE ELSE...)
            ...
            IF (.NOT. QIMG) THEN        !COPY ARRAY OF CENTRAL CELL
               NATOMX1 = NATOMX
            END IF
            IF (QIMG .OR. NATIMX .EQ. 0) THEN
               N = NATOMX
               ALLOCATE (EX(N), EY(N), EZ(N), SOURCE=0.D0)
            END IF
            IF (QFNB .AND. .NOT. QIMG .AND. NATIMX .NE. 0) THEN ! UPDATE AND SAVE NBONDLIST FOR CENTRAL CELL
         ...
         IF (QPME) THEN ! UPDATE 14 EXCLUSION LIST FOR PME
            IF (QDCNB14 .AND. (.NOT. QIMG .OR. NATIMX .EQ. 0)) THEN
...
! WRITE(OUTU,*) "DCME:", QIMG
         ...
         ! ADD POSITIONS AND GEOM. DERIVATIVES FOR DCM CHARGES OF IMAGE
         ! ATOMS:
         IF (QIMG) THEN
            ! WRITE(OUTU,*) "DCME:QIMG"
...
! POLARIZATION ENERGY AND FORCES
         IF (QPOL .AND. (NATIMX .EQ. 0 .OR. QIMG)) THEN ! WAIT UNTIL WE HAVE IMAGES FOR TOTAL EX,EY,EZ
            ...
            DEALLOCATE (EX, EY, EZ)
         END IF !QPOL AND QIMG OR NATIMX.EQ.0
         EEL = EEL + EPOL

         ! OPTIONAL HALGREN BUFFERED 14-7 VDW TERM FOR AMOEBA
         ! COMPATIBILITY:
         IF (QHALG) THEN
         ...
         IF (QPME .AND. .NOT. QIMG) THEN !IF PARTICLE MESH EWALD WASREQUESTED
...
!*******************************************************************************************
   ...
   SUBROUTINE EQQ(NATOMX, NALL, INBL, JNBL, CTOFNB, CTONNB, XQ, YQ, ZQ,&
      ...
      ! GET NONBONDED PAIRS WITHIN CUTOFFS AND CALCULATE CUTOFF
      ! FUNCTIONS, ENERGIES AND FORCES
...
! K. Toepfer, April 2022
      ...
      CTOFNBM2 = CTOFNBM1*CTOFNBM1  ! FASTER TO CALCULATE OUTSIDE THE CNONBOND ATOM-ATOM LOOP
      ...
      ! GET INTERACTING NONBONDED PAIRS FOR CASE WITH AND WITHOUT
      ! POLARIZABLE CENTERS
      ITEMP = 0
...
!*******************************************************************************************
   ...
   SUBROUTINE DCMNBONDS(NATOMX, NALL, INBL, JNBL)
      ...
      ! SUBROUTINE TO UPDATE DCM NONBONDED LIST FOR CENTRAL CELL OR
      ! IMAGES
      ! TODO: STORE NBOND LISTS FOR PERT RUNS INSTEAD OF RE-EVALUATING
      ! FOR EACH CHANGE IN
      ! LAMBDA
      IMPLICIT NONE
...
!*******************************************************************************************
   ...
   SUBROUTINE EHALG(ENB, NATOMX, NALL, INBL, JNBL, X, Y, Z, DX, DY, DZ)
      ...
      ! GET HALGREN BUFFERED 14-7 INTERACTION ENERGY FOR AMOEBA
      ! COMPATIBILITY
...
!*******************************************************************************************
   ...
   SUBROUTINE GETEPOL(NATOMX, INBL, JNBL, CTOFNB, X, Y, Z, DX, DY, DZ, EX, EY, EZ, XQ, YQ, ZQ, DQ, &
      ...
      ! USE ELECTRIC FIELD ARRAY TO EVALUATE POLARIZATION ENERGY AND
      ! DERIVATIVES
      ...
      DO I = 1, NATOMX
         ...
         DO J = ITEMP + 1, INBL(I)  ! EACH ELEMENT J OF THE NONBONDED LIST OF I
            M = ABS(JNBL(J))
            ! FORWARD INTERACTION OF POLARIZABLE CENTER A WITH DCM
            ! CHARGE(S) ON B
            IF (LDP .NE. 0.D0) THEN
               LPAPB = LDP
            ...
            END IF ! LDP.NE.0.D0
            ! ADD REVERSE INTERACTION OF DCM CHARGE(S) ON A WITH
            ! POLARIZABLE CENTER B
            IF (DPOL(M) .NE. 0.D0) THEN
               IF (LDP .NE. 0.D0) THEN
                  LPAPB = LDP*DPOL(M)
               ELSE
   ...
   CONTAINS
      ...
      SUBROUTINE SCUT()
         ...
         ! EVALUATES INTERACTION CUTOFFS AND DERIVATIVES FOR
         ! CHARGE-CHARGE CUTOFF SCHEMES
         IMPLICIT NONE
...
!*******************************************************************************************
   ...
   SUBROUTINE FIELD(NATOMX, INBL, JNBL, CTOFNB, X, Y, Z, DX, DY, DZ, EX, EY, EZ, XQ, YQ, ZQ, DQ, &
      ...
      ! CALCULATE ELECTRIC FIELD AND DERIVATES AT POSITIONS OF
      ! POLARIZABLE NUCLEAR CENTERS
...
! EVALUATES INTERACTION CUTOFFS AND DERIVATIVES FOR NUCLEAR-NUCLEAR
! CUTOFF SCHEMES
      IMPLICIT NONE
...
!*******************************************************************************************
   ...
   SUBROUTINE ADD_DCM_RECIP_F(EWVIRIAL, X, Y, Z)
      ...
      ! ADDS K-SPACE FORCES TO DX,DY,DZ AT CORRECT MOMENT TO CONSERVE
      ! VIRIAL
      ...
      ! ADD_TORQUES APPENDED NUCLEAR TERMS TO END OF KDCD ARRAYS, DO THE
      ! SAME FOR XQAC ETC.
      DO I = 1, NATOMA
         XQAC(I + NALLC) = X(I)
         YQAC(I + NALLC) = Y(I)
...
! NBONDS HAS BEEN CALLED SINCE LAST DCM NBONDLIST UPDATE, SET FLAG TO
! UPDATE DCM LISTS AS WELL
      QDCNB = .TRUE.
      IQDCNB = .TRUE.
      QDCNB14 = .TRUE.
      QFNB = .TRUE.
...
!*******************************************************************************************
   ...
   SUBROUTINE ADD_TORQUES(DCDXA, DCDYA, DCDZA, NATOMX, QCORR)
      ...
      ! APPLY FORCES FROM OFF-CENTER DCM CHARGES TO NUCLEI THAT DEFINE
      ! THEIR FRAMES
      ! NOTE THIS CAN BE PROBLEMATIC FOR THE VIRIAL IF IT WAS ALREADY
      ! CALCULATED FOR THE
      ! OFF-CENTER POSITIONS, CALL THIS ROUTINE AT THE RIGHT TIME OR ADD
      ! A VIRIAL
      ! CORRECTION TO ALLOW NPT CALCULATIONS ETC. (CHECK WITH "TEST
      ! FIRST CRYST" OR
...
! ADD DCM TORQUES
         ...
         IF (NQA(I) .GT. 0) THEN
            ...
            DO J = 0, NQA(I) - 1
               ...
               IF (J1 .NE. 0) THEN ! IF NOT MONATOMIC
                  ! FORCE ON 1ST ATOM USED TO DEFINE LOCAL AXES FROM
                  ! CHARGE K ON I
      ...
      IF (QCORR) THEN
         DO I = 1, NATOMX
            !APPEND NUCLEAR POSITION VIRIAL CORRECTION TO END OF DXDX
            !ARRAYS
         ...
         END DO
         DO I = 1, NALLC
            !OVERWRITE DCM CHARGE FORCE ARRAY WITH VIRIAL CORRECTION FOR
            !EACH CHARGE
            DCDXA(I) = -DCDXA(I)
            DCDYA(I) = -DCDYA(I)
            DCDZA(I) = -DCDZA(I)
...
!*******************************************************************************************
!
! ROUTINE TO PROCESS SYSTEM, DETERMINE WHICH RESIDUES SHOULD HAVE DCM
! CHARGES AND DIVIDE
! UP MOLECULES INTO FRAMES. RESULTS ARE STORED BY CONSTRUCTING THE
...
! NPAT: NUMBER OF PATCHED RESIDUES
      ...
      ! TODO: VERY MEMORY-HUNGRY! ALL DUE TO FIRST ITERATION (BEFORE WE
      ! KNOW NDQ)
      ...
      ! ALLOCATE FOLLOWING VARIABLES HERE, WE ONLY NEED THEM WHILE
      ! READING DEFINITIONS FILE
      ! MAXPAT REFERS TO MAX ALLOWED NO. OF PATCHES (WE CREATE NEW
      ! RESIDUES FOR PACHES)
      ...
      IF (MYNOD .EQ. 0) THEN
            ...
            DO M = 1, RNFRAME(L)       ! LOOP OVER FRAMES
               ...
               IF (WD .EQ. 'BI') THEN
                  ...
                  IF (RFRAMEI(L, M, 2) .EQ. 0) THEN ! MONATOMIC
                     ...
                     END IF
                     CALL WRNDIE(-5, '<DCM> DCMINIT', 'CANNOT USE CBISECTOR Z-AXIS FOR A MONATOMIC. USE BO INSTEAD')
                  END IF
                  IF (RFRAMEI(L, M, 3) .EQ. 0) THEN ! DIATOMIC
                     IF (PRNLEV .GT. 1) THEN
                        WRITE (OUTU, '(A,I5,A)') '<DCM> ERROR FOR RESIDUE ', RESDC(L), ' IN DCM CHARGE FILE'
                     END IF
                     CALL WRNDIE(-5, '<DCM> DCMINIT', 'CANNOT USE CBISECTOR Z-AXIS FOR A DIATOMIC!')
                  END IF
         ...
         ! NOW LOOP OVER ALL RESIDUES IN SYSTEM TO ASSIGN FRAMES,
         ! DISTRIBUTED CHARGES ETC
                        ...
                        IF (IBASE(I) + RFRAMEI(T, K, 3) .LE. NATOMX) THEN
                           ...
                           DO M = 1, 3
                              ...
                              IF (RNQ(T, K, M) .GT. 0) THEN ! IF MTH ATOM IN KTH FRAME HAS DCM CHARGES
                                 ...
                                 DO L = 1, RNQ(T, K, M)
                                    ! DEFINE POSITIONS OF DISTRIBUTED
                                    ! CHARGES IN LOCAL AXIS SYSTEM
                                    ! (A*EI, B*EJ, C*EK)
                                    ! ALLOW ZERO CHARGES FOR
                                    ! THERMODYNAMIC INTEGRATION
                                    NDQ = NDQ + 1
                                    AQ(NDQ) = RAQ(T, K, M, L)
                                    BQ(NDQ) = RBQ(T, K, M, L)
         ...
         ! NOW ZERO REMAINING CHARGES IN SYSTEM (NON-DCM RESIDUES) AND
         ! CONVERT TO DCM NUCLEAR-CENTERED
         DO I = 1, NATOMX
...
!  KEY PARA
      ...
      ! ALLOCATE DERIVATIVE COEEFICIENTS FOR DCM CHARGES, 35X NO. OF
      ! CHARGES IN CASE WE USE IMAGES
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         IF (KERN) THEN
            ...
            !  CALCULATE THE NUMBER OF DISTANCES NEEDED FOR A SYM. DIST.
            !  MATRIX                           
...
!*******************************************************************************************
   ...
   FUNCTION CALC_DTHETADX1(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADX1)
      ...
      DTHETADX1 = (MINONE*(X3*(Y1**2 - TWO*Y1*Y2 + Y2**2 + (Z1 - &
Z2)**2)) + X2*(Y1**2 + Y2*Y3 - Y1*(Y2 + Y3) + (Z1 - Z2)*(Z1 - Z3)) + &
X1*(Y2**2 - Y2*Y3 + Y1*(-Y2 + Y3) - (Z1 - Z2)*(Z2 - Z3)))/(((X1 - X2)**2 &
+ (Y1 - Y2)**2 + (Z1 - Z2)**2)**1.5*SQRT((X2 - X3)**2 + (Y2 - Y3)**2 + &
(Z2 - Z3)**2)*SQRT(ONE - ((X1 - X2)*(-X2 + X3) + (Y1 - Y2)*(-Y2 + Y3) + &
(Z1 - Z2)*(-Z2 + Z3))**2/(((X1 - X2)**2 + (Y1 - Y2)**2 + (Z1 - &
Z2)**2)*((X2 - X3)**2 + (Y2 - Y3)**2 + (Z2 - Z3)**2)))) 
   ...
   FUNCTION CALC_DTHETADX2(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADX2)
      ...
      DTHETADX2 = (MINONE*(((-x1 + TWO*x2 - x3)*((x1 - x2)**2 + (y1 - &
y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2) + &
(-x2 + x3)*((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x1 - x2)*(-x2 &
+ x3) + (y1 - y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3)) + (x1 - x2)*((x2 - &
x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)*((x1 - x2)*(-x2 + x3) + (y1 - &
y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3)))/(((x1 - x2)**2 + (y1 - y2)**2 + &
(z1 - z2)**2)**1.5*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - &
z3)**2)**1.5*Sqrt(1 - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) + (z1 &
- z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x2 &
- x3)**2 + (y2 - y3)**2 + (z2 - z3)**2))))))
   ...
   FUNCTION CALC_DTHETADX3(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADX3)
      ...
      DTHETADX3 = ((x3*(y2**2 - y2*y3 + y1*(-y2 + y3) - (z1 - z2)*(z2 - &
z3)) - x1*(y2**2 - TWO*y2*y3 + y3**2 + (z2 - z3)**2) + x2*(y1*(y2 - y3) &
- y2*y3 + y3**2 + z1*z2 - z1*z3 - z2*z3 + z3**2)) / (Sqrt((x1 - x2)**2 + &
(y1 - y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - &
z3)**2)**1.5*Sqrt(ONE - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) + &
(z1 - z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 - &
z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2))))) 
...
!!!!!!  Y derivatives
   ...
   FUNCTION CALC_DTHETADY1(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADY1)
      ...
      DTHETADY1 = ((x2*x3*(-y1 + y2) - x1*(x3*(-y1 + y2) + x2*(y1 + y2 -&
TWO*y3)) + x2**2*(y1 - y3) + x1**2*(y2 - y3) + (z1 - z2)*(y2*z1 - y3*z1&
- y1*z2 + y3*z2 + y1*z3 - y2*z3)) / (((x1 - x2)**2 + (y1 - y2)**2 + (z1&
- z2)**2)**1.5*Sqrt((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)*Sqrt(1 -&
((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) + (z1 - z2)*(-z2 +&
z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 +&
(y2 - y3)**2 + (z2 - z3)**2)))))
   ...
   FUNCTION CALC_DTHETADY2(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADY2)
      ...
      DTHETADY2 = MINONE * (((-y1 + TWO*y2 - y3)*((x1 - x2)**2 + (y1 -& 
y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2) +&
(-y2 + y3)*((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x1 - x2)*(-x2&
+ x3) + (y1 - y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3)) +  (y1 - y2)*((x2 -&
x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)*((x1 - x2)*(-x2 + x3) + (y1 -&
y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3)))/(((x1 - x2)**2 + (y1 - y2)**2 +&
(z1 - z2)**2)**1.5*((x2 - x3)**2 + (y2 - y3)**2 + (z2 -&
z3)**2)**1.5*Sqrt(ONE - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) +&
...
z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)))))
   ...
   FUNCTION CALC_DTHETADY3(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADY3)
      ...
      DTHETADY3 =  (x3**2*(-y1 + y2) + x2*x3*(TWO*y1 - y2 - y3) +&
x1*x2*(y2 - y3) + x2**2*(-y1 + y3) + x1*x3*(-y2 + y3) + (z2 -&
z3)*(y3*(-z1 + z2) + y2*(z1 - z3) + y1*(-z2 + z3))) / (Sqrt((x1 - x2)**2&
+ (y1 - y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 -&
z3)**2)**1.5*Sqrt(1 - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) + (z1&
- z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x2&
- x3)**2 + (y2 - y3)**2 + (z2 - z3)**2))))
...
!!!!!!  Z derivatives
   ...
   FUNCTION CALC_DTHETADZ1(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADZ1)
      ...
      DTHETADZ1 = ((x2*x3*(-z1 + z2) - x1*(x3*(-z1 + z2) + x2*(z1 + z2 -&
TWO*z3)) + x2**2*(z1 - z3) + x1**2*(z2 - z3) + (y1 - y2)*(-(y2*z1) +&
y3*z1 + y1*z2 - y3*z2 - y1*z3 + y2*z3)) / (((x1 - x2)**2 + (y1 - y2)**2&
+ (z1 - z2)**2)**1.5*Sqrt((x2 - x3)**2 + (y2 - y3)**2 + (z2 -&
z3)**2)*Sqrt(ONE - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) + (z1 -&
z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)*((x2 -&
x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)))))
   ...
   FUNCTION CALC_DTHETADZ2(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADZ2)
      ...
      DTHETADZ2 = (MINONE*((((x1 - x2)**2 + (y1 - y2)**2 + (z1 -&
z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)*(-z1 + TWO*z2 - z3)&
+ (z1 - z2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)*((x1 - x2)*(-x2&
+ x3) + (y1 - y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3)) + ((x1 - x2)**2 +&
(y1 - y2)**2 + (z1 - z2)**2)*(-z2 + z3)*((x1 - x2)*(-x2 + x3) + (y1 -&
y2)*(-y2 + y3) + (z1 - z2)*(-z2 + z3))) / (((x1 - x2)**2 + (y1 - y2)**2&
+ (z1 - z2)**2)**1.5*((x2 - x3)**2 + (y2 - y3)**2 + (z2 -&
z3)**2)**1.5*Sqrt(ONE - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) +&
(z1 - z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 -&
z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2))))))
   ...
   FUNCTION CALC_DTHETADZ3(x1, x2, x3, y1, y2, y3, z1, z2, z3) RESULT(DTHETADZ3)
      ...
      DTHETADZ3 = ((x3**2*(-z1 + z2) + x2*x3*(TWO*z1 - z2 - z3) +&
x1*x2*(z2 - z3) + x2**2*(-z1 + z3) + x1*x3*(-z2 + z3) - (y2 -&
y3)*(y3*(-z1 + z2) + y2*(z1 - z3) + y1*(-z2 + z3))) / (Sqrt((x1 - x2)**2&
+ (y1 - y2)**2 + (z1 - z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 -&
z3)**2)**1.5*Sqrt(ONE - ((x1 - x2)*(-x2 + x3) + (y1 - y2)*(-y2 + y3) +&
(z1 - z2)*(-z2 + z3))**2/(((x1 - x2)**2 + (y1 - y2)**2 + (z1 -&
z2)**2)*((x2 - x3)**2 + (y2 - y3)**2 + (z2 - z3)**2)))))
...
! 1. MODIFY RESIDUE ATOMS THAT ARE PART OF PATCH BY SETTING CHARGES TO
! PATCH
! VALUES AND DELETING EXISTING CHARGES
! 2. DELETE ATOMS FROM DCM RESIDUE THAT WERE DELETED BY PATCH, CORRECT
! ATOM
! NUMBERING & REMOVE FRAMES CONTAINING DELETED ATOMS (EXIT IF NOT
! POSSIBLE)
! 3. ADD ATOMS TO START OR END OF RESIDUE THAT WERE ADDED BY PATCH AND
! CORRECT
! ATOM NUMBERING. ADD ATOMS TO NEW FRAME.
! 4. CORRECT TOTAL CHARGE BY DIVIDING REMAINDER BETWEEN ALL CHARGE SITES
! IN
! RESIDUE
!
!*******************************************************************************************
...
! CHECK THAT ATOMS AREN'T REQUIRED BY FRAMES WITH DCM CHARGES ANYWHERE,
! OTHERWISE
...
! EASIEST TO CORRECT TOTAL CHARGE OVER PATCH GROUP ATOMS AS THEY HAVE
! ONLY CENTRAL CHARGES
      DO I = 1, RNFRAME(NDCMRES)
         DO J = 1, 3
            IF (QPATCHF) THEN
               DO K = 1, DCMNPATF
...
!*******************************************************************************************
!
! SUBROUTINE TO INITIALIZE DCM FRAMES, CHARGES ETC. FOR IMAGE MOLECULES
! IDENTIFIED IN
! UPIMG.SRC. THIS ROUTINE IS CALLED DIRECTLY FROM THERE AND NOT AT ALL
! DURING SIMULATIONS
...
!*******************************************************************************************
   ...
   SUBROUTINE IMGINIT(NATOMI, IS, IL)
      ! NATOMI FROM UPIMAGE.SRC, CONTAINS START INDEX FOR NEXT GROUP OF
      ! IMAGE ATOMS TO BE ADDED
      ! IS AND IL ALSO FROM UPIMAGE.SRC. THEY CONTAIN 1ST AND LAST ATOM
         ...
         DO K = 0, NQA(J) - 1
            ...
            IF (KERN) THEN !.OR. KERN
               IF ((FLXFRS(AFRAME(IS+I,1)) .EQ. 1) .AND. (NATMK .GT. 3)) THEN
                  ...
                  ! WRITE(OUTU,*) "J-IREF(J)", J-IREF(J), "IDQ(J)+K",
                  ! IDQ(J)+K, "IDQ(IS+I)+K", IDQ(IS+I)+K
                  ! WRITE(OUTU,*) "XKERNATMS(IDQ(IS+I)+K,1)",
                  ! XKERNATMS(IDQ(IS+I)+K,1), "K", K
                  ! WRITE(OUTU,*) "XKERNATMS(IDQ(J)+K,1)",
                  ! XKERNATMS(IDQ(J)+K,1)
               END IF
            END IF !KERN
         ...
         DO L = 1, NF ! QUITE HEAVY APPROACH: CAN OPTIMIZE HERE!
            IF (FRAMEI(L, 1) .EQ. IREF(J)) THEN ! THIS IS 1ST ATOM IN SOME FRAME, CREATE NEW FRAME FOR THIS IMAGE ATOM
              !  WRITE(OUTU,*) "RESIZING IMAGE FRAMES"
               INF = INF + 1
               ! CHECK WHETHER WE NEED TO ENLARGE ARRAYS TO ACCOMMODATE
               ! NEW FRAME
...
!
!*******************************************************************************************
! PROPERLY HANDLE USER CHOOSING LARGE CUTOFFS FOR PERIODIC SYSTEMS
! (SIMILAR TO BOX SIZE)
...
!*******************************************************************************************
!
! ROUTINE CALLED ONCE PER TIME STEP TO CALCULATE THE LOCAL AXES REQUIRED
! TO PLACE DCM
! CHARGES RELATIVE TO NUCLEAR COORDINATES OF EACH FRAME. AXIS
! DERIVATIVES ARE ALSO
! CALCULATED, TO BE USED TO EVALUATE FORCES IN THE DCME SUBROUTINE.
! DEPSITE ITS LENGTH
! AND COMPLEXITY, RELATIVELY LITTLE TIME IS SPENT HERE AS CALCULATIONS
! ARE DOMINATED BY
...
! FIND LOCAL AXES AND DERIVATIVES
      ...
      ! THIS SUBROUTINE IS COMPLEX AND COULD BE WRITTEN IN DIFFERENT
      ! WAYS. THE AIM IS TO
      ! CALCULATE THE GEOMETRIC DERIVATIVES OF EACH LOCAL AXIS UNIT
      ! VECTOR WITH RESPECT TO EACH
      ! ATOMIC COORDINATE IN A FRAME. THERE ARE 3 NON-COLINEAR ATOMS IN
      ! EACH FRAME, EACH WITH THEIR OWN LOCAL
      ! AXIS SYSTEM. THIS MEANS WE NEED PARTIAL DERIVATIVES OF ATOM 1'S
      ! LOCAL X,Y AND Z AXES WRT ATOM 1,2,3,
      ! THE SAME FOR ATOMS 2 AND 3. EACH LOCAL AXIS VECTOR HAS 3
      ! COMPONENTS IN THE GLOBAL AXIS
      ! SYSTEM, SO WE NEED DXI/DRA, DXJ/DRA, DXK/DRA FOR THE LOCAL
      ! X-AXIS VECTOR OF ATOM 1. THE X-AXIS
      ! VECTOR COMPONENTS I,J,K ARE ITS X,Y, AND Z COMPONENTS IN THE
      ! GLOBAL AXIS SYSTEM. SO FOR
      ! ATOM 1'S X-AXIS WE NEED 9 PARTIAL DERIVATIVES: DX/DRA ->
      ! DXI/DA1X, DXI/DA1Y, DXI/DA1Z,
      ! DXI/DA2X ... DXI/DA3Z. DA1X IS THE X-COORD OF ATOM 1. WE THEN
      ! NEED ATOM 1'S Y AND Z-AXIS
      ! DERIVATIVES, GIVING 27 PARTIAL DERIVATIVES IN TOTAL. THE SAME
      ! FOR ATOMS 2 AND 3, WHICH HAVE THEIR
      ! OWN LOCAL AXIS VECTORS, LEADING TO 81 PARTIAL DERIVATIVES IN
      ! TOTAL, NAMED EG DXEIDA1X(NF,3).
      !
      ! WHEN WE EVALUATE PARTIAL DERIVATIVES BELOW THE CODE IS WRITTEN
      ! IN A VERY GENERAL BUT POTENTIALLY
      ! CONFUSING FASHION. THE NOTATION OF THE DERIVATIVES IS THEREFORE
      ! IMPORTANT TO UNDERSTAND:
      !
      ! DZEIDA1X(FR,1): THIS NOTATION MEANS THAT WE ARE EVALUATING A
      ! PARTIAL DERIVATIVE FOR ATOM 1 IN
      ! THE FRAME 'FR', AS SHOWN BY THE ARRAY INDICES (FR,1). DZEI SHOWS
      ! THAT WE WANT A PARTIAL DERIVATIVE
      ! OF THE 'I' (X) COMPONENT OF THE LOCAL Z-AXIS UNIT VECTOR IN THIS
      ! ATOM'S LOCAL COORDINATE SYSTEM
      ! (SEE DCM JCTC PUBLICATION FOR DETAILS OF AXIS SYSTEM). THE DA1X
      ! SHOWS THAT WE ARE EVALUATING THE
      ! DERIVATIVE WITH RESPECT TO THE X-COORDINATE OF ATOM 1. SO FAR SO
      ! LOGICAL. THE COMPLICATION COMES
      ! IN THAT WHEN WE EVALUATE PARTIAL DERIVATIVES OF ATOM 1'S LOCAL
      ! AXES, DA1 REFERS TO ATOM 1, DA2 TO
      ! ATOM 2, DA3 TO ATOM 3 AS EXPECTED. WHEN WE CALCULATE ATOM 2'S
      ! PARTIAL DERIVATIVES (FR,2), DA1
      ! IS NOW ATOM 2, DA2 IS ATOM 1, DA3 IS ATOM 3. FOR ATOM 3'S LOCAL
      ! AXES (FR,3) DA1 IS ATOM 3, DA2
      ! IS ATOM 2, DA3 IS ATOM 1. THIS CONVENTION COULD (AND PROBABLY
      ! SHOULD) BE CHANGED!
            ...
            IF (FLXFRS(FR) .EQ. 1) THEN ! IF THIS FRAME NEEDS FLUX
               ...
               DO M = 1, 3 ! LOOP OVER ATOMS IN THE FRAME
                  DO K = 0, NQ(FR, M) - 1 ! LOOP OVER CHARGES ASSOC. W/ THE ATOM IN THAT FRAME
                     ...
                     ! WRITE(OUTU,*) IA, IB, IC, AQ(L), BQ(L), CQ(L),
                     ! DQ(L)
      ...
      IF (FLUX) THEN
         DO FR = 1, NF ! LOOP OVER FRAMES
            IF (FLXFRS(FR) .EQ. 1) THEN ! IF THIS FRAME NEEDS FLUX
               ...
               ! DTX1(FR) = CALC_DTHETADX1(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTY1(FR) = CALC_DTHETADY1(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTZ1(FR) = CALC_DTHETADZ1(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTX2(FR) = CALC_DTHETADX2(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTY2(FR) = CALC_DTHETADY2(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTZ2(FR) = CALC_DTHETADZ2(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTX3(FR) = CALC_DTHETADX3(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTY3(FR) = CALC_DTHETADY3(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG
               ! DTZ3(FR) = CALC_DTHETADZ3(FAX1, FAX2, FAX3, FAY1, FAY2,
               ! FAY3, FAZ1, FAZ2, FAZ3)*RAD2DEG


               DO M = 1, 3 ! LOOP OVER ATOMS IN THE FRAME
                  !  CALCULATE THE ANGLE DERIVATIVE ONCE SINCE IT
                  !  APPLIES TO EACH ATOM OF THE FRAME DIFFERENTLY
                  IF (NQ(FR, M) .GT. 0) THEN !  CHECK IF THERE ARE CHARGES IN THAT FRAME
                     DO K = 0, NQ(FR, M) - 1 ! LOOP OVER CHARGES ASSOC W/ THE ATOM IN THAT FRAME
                        ! RECALCULATE LOCAL AXIS POSITION BASED ON FIT
                        L = IDQ(FRAMEI(FR, M)) + K

                        ! AQ(L) = FPX1(L)*ANGLEVAL + FPX2(L)*THTH +
                        ! FPX3(L)*THTHTH + FPX4(L)
                        ! BQ(L) = FPY1(L)*ANGLEVAL + FPY2(L)*THTH +
                        ! FPY3(L)*THTHTH + FPY4(L)
                        ! CQ(L) = FPZ1(L)*ANGLEVAL + FPZ2(L)*THTH +
                        ! FPZ3(L)*THTHTH + FPZ4(L)

                        ! ! ASSIGN DERIVATIVES
                        ! DADT = FPX1(L) + TWO*FPX2(L)*ANGLEVAL +
                        ! THREE*FPX3(L)*THTH
                        ! !POLYX3DERIV(ANGLEVAL,FPX1(L),FPX2(L),FPX3(L))
                        ! DBDT = FPY1(L) + TWO*FPY2(L)*ANGLEVAL +
                        ! THREE*FPY3(L)*THTH
                        ! !POLYX3DERIV(ANGLEVAL,FPY1(L),FPY2(L),FPY3(L))
                        ! DCDT = FPZ1(L) + TWO*FPZ2(L)*ANGLEVAL +
                        ! THREE*FPZ3(L)*THTH
                        ! !POLYX3DERIV(ANGLEVAL,FPZ1(L),FPZ2(L),FPZ3(L))
      ...
      DO FR = 1, NF ! LOOP OVER FRAMES
         ...
         IF (FRTYP(FR) .EQ. 0) THEN ! IF "BOND"-TYPE AXIS SYSTEM FOR THIS FRAME
            ...
            IF (ATM3 .NE. 0) THEN ! NOT DIATOMIC
               ...
               ! EY
               ! DEFINE LOCAL Y-AXIS = BOND ATM2-ATM1 CROSS BOND
               ! ATM2-ATM3
            ...
            FACA = -1.D0/(RB1*RB12)
            FACB = FACA*(-B1X)
            ! DZEIDA1X=DERIVATIVE OF LOCAL Z-AXIS UNIT VECTOR
            ! I-COMPONENT (ZEI) WRT ATOM1 X-COOR
            ...
            ! DZEIDA2X=DERIVATIVE OF LOCAL Z-AXIS UNIT VECTOR
            ! I-COMPONENT (ZEI) WRT ATOM2 X-COOR
            ...
            ! ATOM 2 DZEIDA1X=DERIVATIVE OF LOCAL Z-AXIS UNIT VECTOR
            ! I-COMPONENT (ZEI) WRT ATOM1 X-COOR
            ...
            IF (ATM3 .NE. 0) THEN ! NOT DIATOMIC
               FACC = -1.D0/(RB2*RB22)
               FACB = FACC*(-B2X)
               ! ATOM 3 DZEIDA1X=DERIVATIVE OF LOCAL Z-AXIS UNIT VECTOR
               ! I-COMPONENT (ZEI) WRT ATOM1 X-COOR
                  ...
                  DO K = 0, NQ(FR, M) - 1
                     ...
                     IF (DPAIR(L) .GE. 0) THEN
                        IF (FRAMEI(FR, 3) .NE. 0) THEN
                           !  Calculates the translation from the
                           !  nuclear centre
                     ...
                     IF (DPAIR(L) .GT. 0) THEN  ! IS THIS MAKING THECODE ANY FASTER
                        J = DPAIR(L)
                     ...
                     IF (KERN) THEN !.OR. KERN
                        IF (FLXFRS(FR) .EQ. 1) THEN ! IF THIS FRAMENEEDS FLUX

                           IF (NATMK .GT. 3) THEN
                              DO J = 1, NATMK - 3

                                 ! WRITE(OUTU,*) "FR", FR, "J", J, "L",
                                 ! L
...
! FIND LOCAL AXES AND DERIVATIVES
      ...
      ! OF THE 'I' (X) COMPONENT OF THE LOCAL Z-AXIS UNIT VECTOR IN THIS
      ! ATOM'S LOCAL COORDINATE SYSTEM
      ! (SEE DCM JCTC PUBLICATION FOR DETAILS OF AXIS SYSTEM, IN THE
      ! AXIS2 ROUTINE THE ONLY DIFFERENCE IS THAT
      ! ATOM 2'S Z-AXIS IS THE BOND1-BOND2 BISECTOR, RATHER THAN SIMPLY
      ! BOND1). THE DA1X SHOWS THAT WE
      ! ARE EVALUATING THE DERIVATIVE WITH RESPECT TO THE X-COORDINATE
      ! OF ATOM 1. SO FAR SO LOGICAL. THE
      ! COMPLICATION COMES IN THAT WHEN WE EVALUATE PARTIAL DERIVATIVES
      ! OF ATOM 1'S LOCAL AXES, DA1
      ! REFERS TO ATOM 1, DA2 TO ATOM 2, DA3 TO ATOM 3 AS EXPECTED. WHEN
      ! WE CALCULATE ATOM 2'S PARTIAL
      ! DERIVATIVES (FR,2), DA1 IS NOW ATOM 2, DA2 IS ATOM 1, DA3 IS
      ! ATOM 3. FOR ATOM 3'S LOCAL AXES (FR,3)
      ! DA1 IS ATOM 3, DA2 IS ATOM 2, DA3 IS ATOM 1. THIS CONVENTION
      ! COULD (AND PROBABLY SHOULD) BE CHANGED!
      ...
      DO FR = 1, NF ! LOOP OVER FRAMES
         IF (FRTYP(FR) .EQ. 1) THEN ! IF "BOND"-TYPE AXIS SYSTEM FOR THIS FRAME
            ...
            ! DZEIDA2X=DERIVATIVE OF LOCAL ATOM 2 (FR,2) Z-AXIS UNIT
            ! VECTOR I-COMPONENT (ZEI)
            ! WRT ATOM2 X-COOR
...
!*******************************************************************************************
!
! NO LONGER USED, THIS SUBROUTINE FINDS GEOMETRICALLY RELATED DCM
! CHARGES THAT ALLOW SOME VERY
...
!*******************************************************************************************
!
! WRITE AN XYZ FILE WITH DCM CHARGE COORDINATES IN GLOBAL AXIS, PLUS
! CHARGES IN A.U.
!
!*******************************************************************************************
...
!*******************************************************************************************
   ...
   SUBROUTINE DCMDIPOFILE(UN, XQ, YQ, ZQ, DQ, X, Y, Z, NATOMX, NALL)
      ...
      ! CALCULATE DIPOLE MOMENT OF DCM CHARGES FOR A GIVEN ATOM
      ! SELECTION AND WRITE
            ...
            IF (DYNAMQ) THEN
               IF (STEPCOUNT > 1) THEN
                  IF (ISTEP < 100 .AND. ISTEP .GE. 0 .AND. MOD(ISTEP,NSAVDIPO) .EQ. 0) THEN
                     ...
                     ! WRITE DIPOLE FILE
                     !##########################
                     WRITE (UN, '(2X,3(2X,F8.5))') XMU, YMU, ZMU
...
!*******************************************************************************************
!
! BACKUP CURRENT DCM CHARGES CORRESPONDING TO LAMBDA=0 STATE IN
! THERMODYNAMIC INTEGRATION
! CALCS, ALSO COPY CENTRAL CHARGES BACK TO CG ARRAY FOR CHARMM. THIS
! ROUTINE SHOULD BE
...
!*******************************************************************************************
!
! APPLY A SECOND SET OF DCM CHARGES CORRESPONDING TO AN END STATE
! (LAMBDA=1) FOR
! THERMODYNAMIC INTEGRATION. THE ORIGINAL DCM CHARGES SHOULD HAVE BEEN
! STORED IN AN ARRAY
! BY DCMCHGCOPY.
!
!*******************************************************************************************
   ...
   SUBROUTINE DCENDSTATE()
      ...
      IF (NDQ .NE. TNDQ) THEN
         ...
         END IF
#endif
         CALL WRNDIE(-1, '<DCM> DCENDSTATE', 'CHANGE IN # DCM CHARGES IN CDCENDSTATE')
      END IF
...
!*******************************************************************************************
!
! SWITCH TO THERMODYNAMIC INTEGRATION INITIAL (LAMBDA=0) OR FINAL
! (LAMBDA=1) STATE
!
!*******************************************************************************************
...
!*******************************************************************************************
!
! WITE OUT MULTIPOLE MOMENTS GENERATED BY DCM CHARGES (A.U.) IN GDMA
! PUNCH FILE FORMAT
!
!*******************************************************************************************
